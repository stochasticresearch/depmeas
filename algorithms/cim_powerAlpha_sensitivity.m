function [powerCurve] = cim_powerAlpha_sensitivity(cimfunc, M, minscanincrVal, alphasToTest)
...
powerCurve = zeros(length(alphasToTest),numDepTests,num_noise);
    ...
    for lIdx=1:length(noiseVec)
        ...
        for typ=1:numDepTests
            ...
            parfor ii=1:nsim_null
                ...
                cimNull(ii)   = cimfunc(x,y,minscanincrVal,alphaVal);
            end
            ...
            parfor ii=1:nsim_alt
                ...
                % calculate the metrics
                cimAlt(ii)   = cimfunc(x,y,minscanincrVal,alphaVal);
            end

            % compute the power
            powerCurve(alphaIdx, typ, lIdx)   = sum(cimAlt > cim_cut)/nsim_alt;
        end
    end
end
